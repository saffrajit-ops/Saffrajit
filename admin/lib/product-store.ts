...
interface ProductState {
  ...
  isLoading: boolean
  error: string | null
  fetchProducts: (token: string, page?: number, limit?: number) => Promise<void>
...
export const useProductStore = create<ProductState>((set) => ({
  ...
  fetchProducts: async (token: string, page = 1, limit = 100) => {
    set({ isLoading: true, error: null })
    try {
      const response = await fetch(`${API_BASE}/admin/products?page=${page}&limit=${limit}&sort=-createdAt`, {
        headers: {
