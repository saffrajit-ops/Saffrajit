...
import { useProductStore, BulkUploadResult } from "@/lib/product-store"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export function BulkUploadDialog({ open, onOpenChange, onSuccess }: BulkUploadDialogProps) {
  ...
  const { bulkUploadProducts } = useProductStore()
  ...
  const handleUpload = async () => {
    ...
    try {
      const uploadResult = await bulkUploadProducts(file, accessToken, (prog) => {
        setProgress(prog)
      })
      ...
      if (uploadResult.success.length > 0) {
        toast.success("Upload Complete", {
          description: `Successfully uploaded ${uploadResult.success.length} of ${uploadResult.total} products`
        })
        onSuccess()
      }
      ...
      if (uploadResult.failed.length === uploadResult.total) {
        toast.error("Upload Failed", {
          description: "All products failed to upload. Please check the errors below."
        })
      }
    } catch (error) {
      toast.error("Upload Failed", {
  ...
  const handleCancel = () => {
    ...
    toast.info("Upload Cancelled", {
      description: "Product upload has been cancelled"
    })
  }
  ...
  const handleDownloadTemplate = () => {
    const templateUrl = "/templates/product-bulk-upload-template.xlsx"
